...
class CategoryMySQLGatewayTest {
    ...
    public void givenAValidCategory_whenCallsCreate_shouldReturnANewCategory() {
        final var exoectedName = "Filmes";
        final var exoectedDescription = "A categoria mais assitida";
        final var exoectedIsActive = true;

        final var aCategory = Category.newCategory(exoectedName, exoectedDescription, exoectedIsActive);

        Assertions.assertEquals(0, categoryRepository.count());

        final var actualCcategory = categoryGateway.create(aCategory);

        Assertions.assertEquals(1, categoryRepository.count());

        Assertions.assertEquals(exoectedName, actualCcategory.getName());
        Assertions.assertEquals(exoectedDescription, actualCcategory.getDescription());
        Assertions.assertEquals(exoectedIsActive, actualCcategory.isActive());
        Assertions.assertEquals(aCategory.getId(), actualCcategory.getId());
        Assertions.assertEquals(aCategory.getCreatedAt(), actualCcategory.getCreatedAt());
        Assertions.assertEquals(aCategory.getUpdatedAt(), actualCcategory.getUpdatedAt());
        Assertions.assertEquals(aCategory.getDeletedAt(), actualCcategory.getDeletedAt());
        Assertions.assertNull(actualCcategory.getDeletedAt());

        final var actualEntity = categoryRepository.findById(aCategory.getId().getValue()).get();

        Assertions.assertEquals(exoectedName, actualEntity.getName());
        Assertions.assertEquals(exoectedDescription, actualEntity.getDescription());
        Assertions.assertEquals(exoectedIsActive, actualEntity.isActive());
